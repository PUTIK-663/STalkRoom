...
if ('WebSocket' in window) {
    websocket = new WebSocket("ws://"+(window.location.href).substring(6)+"websocket");
    //websocket = new WebSocket("ws://www.hnuster.site:80/websocket");
} else {
...
websocket.addEventListener('message',function (event){
    ...
    switch (messageJson["type"]){
        case "normal":
            //创建聊天气泡
            createMsgDiv(messageJson,"normal","1");
            break;
        case "system":
            //系统消息只用于发送用户基本信息
            //-1为系统保留数字，-2位在线人数，-3为用户名
            if (messageJson["sender"]==="-2"){
                document.getElementById("nowOnlinePeople").innerText =
                    "当前在线人数："+messageJson["content"];
            }
            break;
    }
});
...
function createMsgDiv(messageJson,type,who){
    messageJson["content"] = formatText(messageJson["content"]);
